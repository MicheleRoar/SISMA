...
(() => {
  ...
  async function createSpotifyPlaylistWithTracks(trackIds) {
    ...
    const date = new Date();
    const name = `SISMA • ${date.toISOString().slice(0, 10)} • ${trackIds.length} tracks`;
  ...
  async function onAddToSpotify() {
    ...
    // Se la tabella esiste ma nessuna track è selezionata → errore
    const hasCheckboxes = document.querySelectorAll(".include-track").length > 0;

    if (!trackIds.length && hasCheckboxes) {
      throw new Error("No tracks selected. Tick at least one track before exporting.");
    }

    // fallback solo se non esiste la tabella
    if (!trackIds.length) {
